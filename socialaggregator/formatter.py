...
class ResourceFormatterBase(object):
    """
    This is a dummy resource formatter, its method ``render`` will
    simply return the given instance, no format is applied here
...
class ResourceFormatterDefault(ResourceFormatterBase):
    """
    Default formatter, its goal is to unify various data types from
    social networks in only one format to avoid to manage many various
    cases with all social networks

    This formatter is heavily linked to Foundation5 and private CSS
    that is not shipped in the app, so you will have to implement it
    if needed
    ...
    type_classes = {
        'facebook_fanpage': 'facebook',
        'twitter': 'twitter',
        'instagram': 'instagram',
        'article': 'article',
        'wordpress_rss': 'article',
        'pinterest': 'pinterest',
        'youtube': 'youtube',
